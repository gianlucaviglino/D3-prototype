...
define('utility', ['jquery', 'modernizr'], function ($) {
  ...
  return Utility = {
    ...
    toQuerParams: function (source,separator) {
...
define('fi_select', ['jquery', 'utility'], function ($, Utility) {
  ...
  $.fn.fi_select = function () {
    ...
    var select_item = $('.fi-select > option[value = ' + lang + ']', this);

    var list_item = $('.dropdown-menu > li[data-value = ' + lang + ']', this);

    if ($(".fi-select").parents(".fi-nav__languages-menu-wrap").length) {
      $('.fi-nav__languages-menu-wrap .fi-selected-item').text(select_item.first().text());
    }
    ...
    $(this).on('focusout', function () {
      if (!$('.dropdown-menu > li:hover', this).length)
    ...
    $('.dropdown-menu > li', this).on('click', function (e) {
      e.preventDefault();
      var thisDD = $(this).parents(".fi-select-wrapper");
      var isTT = (thisDD.find(".fi-select").parents(".fi-tt-cities-dropdown-wrapper").length > 0);
      if (!isTT) {
        $('.fi-selected-item', thisDD).text($(this).text());
      }
      $('.dropdown-menu > li', thisDD).removeClass('active');

      list_item = $(this);
      select_item = $('.fi-select > option', thisDD).eq(list_item.index());
      select_item.prop('selected', true);
      list_item.addClass('active');
      var item = list_item.find('a');
      if (!isTT) {
        $(_this).trigger('fi_select.change', [{ url: item.attr('href'), item: item.attr('value'), }]);
      } else {
        setWinLocation(item);
      }
    });

    $('.fi-select', this).on('change', function () {
      console.log("on change");
      var thisDD = $(this).parents(".fi-select-wrapper");
      var isTT = isThophyTourDropDown(thisDD);
      var index_selected = $(this).find(":selected").index();
      var item = thisDD.find('.dropdown-menu > li', _this).eq(index_selected).find('a');
      if (!isTT) {
        $(this).trigger('fi_select.change', [{ url: item.attr('href'), item: item.attr('value') }]);
      } else {
        setWinLocation(item);
      }
    });

    function isThophyTourDropDown(thisDD) {
      var isTT = (thisDD.find(".fi-select").parents(".fi-tt-cities-dropdown-wrapper").length > 0);
      return isTT;
    }

    function setWinLocation(item) {
      var _hash = item.attr('href');
      if (_hash) {
  ...
  var Adv = {
    loadAdv: function () {
      ...
      googletag.cmd.push(function () {
        var $dfpslots = $(".fi-adv-placeholder").filter(":visible");
        if ($dfpslots.length) {
...
define('sharing', ['jquery'], function ($) {
  ...
  var Sharing = {
    ...
    shareFacebook: function (e) {
      ...
      Utility.log(_sharingURL);

      window.open(['http://www.facebook.com/sharer/sharer.php?u=', _sharingURL].join(''), '', Sharing._sharingParam);
    },
    ...
    shareTwitter: function (e) {
      ...
      window.open(['http://twitter.com/intent/tweet?url=', _sharingURL, '&text=', Sharing._openGraph["og:title"], " ", _sharingURL].join(''), '', Sharing._sharingParam);
    },
    ...
    shareGoogle: function (e) {
      ...
      window.open(['https://plus.google.com/share?url=', _sharingURL].join(''), '', Sharing._sharingParam);
    },
    ...
    , getSharableURL: function (url, channel) {
      ...
      Utility.log("_cid_campaign: ", _cid_campaign);
...
require(['jquery', 'utility', 'headroom', 'waypoints', 'fi_select', 'placeholder', 'gridview',
          ...
          'geodetect', 'cookiebanner', 'adv', 'sharing'], function ($, Utility, Headroom) {
              //on ready dom
              $(document).ready(function () {
                  var $body = $('body');
                  ...
                  $('.fi-select-wrapper').fi_select();

                  $('.fi-select-wrapper').on('fi_select.change', function (e, data) {
                      if (data.url.indexOf("#") !== -1) {
                          window.location.hash = data.url;
                      } else {
                          window.location.href = data.url.replace(/\/+$/, "") + location.pathname + location.search;
                      }
                  ...
                  if ($pageHeader.length > 0) {
                      ...
                      $('.nav.navbar-nav.fi-page-nav').on('clear.bs.scrollspy', function (e, activeTarget) {
                  ...
                  var params = Utility.toQuerParams(window.location.href);
